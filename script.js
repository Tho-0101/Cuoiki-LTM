document.addEventListener('DOMContentLoaded', () => {
    ...
    const paymentShowtimeInfo = document.getElementById('payment-showtime-info');
    ...
    async function fetchShowtimes(date) {
        try {
            ...
            alert(error.message);
            movieShowtimesContainer.innerHTML = `<p style="text-align:center; color: #aaa;">Không có suất chiếu cho ngày này.</p>`;
        }
    }
    ...
    function renderMyTicketPage() {
        ...
        if (state.bookedTickets.length > 0) {
            ...
            ticketDisplayArea.innerHTML = `<p style="text-align:center; color: #aaa;">Bạn chưa đặt vé nào. Hãy chọn phim và đặt vé nhé!</p>`;
        }
    }
    ...
    function renderPaymentPage() {
        ...
        const numSeats = state.selectedSeats.size;
        const totalPrice = numSeats * SEAT_PRICE;
        
        paymentMovieTitle.innerText = state.selectedMovie.ten;
        paymentShowtimeInfo.innerText = `Suất chiếu: ${state.selectedShowtime.gio} - Ngày: ${state.selectedShowtime.ngay}`;
        paymentSeatsList.innerText = Array.from(state.selectedSeats).sort().join(', ');
        paymentTotalPrice.innerText = totalPrice.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' });
    ...
    loginBtn.addEventListener('click', () => {
        ...
        state.bookedTickets = savedTickets ? JSON.parse(savedTickets) : [];
    ...
    logoutBtn.addEventListener('click', () => {
        ...
        state = { ...state, userName: '', userPhone: '', bookedTickets: [] };
        nameInput.value = '';
        sdtInput.value = '';
    ...
    backToBookingBtn.addEventListener('click', () => navigateTo(bookingPage));
    ...
    async function bookTickets() {
        ...
        try {
            ...
            alert('Đã xảy ra lỗi hệ thống khi đặt vé.');
            console.error("Booking Error:", error);
        }
    }
    finalBookBtn.addEventListener('click', bookTickets);
